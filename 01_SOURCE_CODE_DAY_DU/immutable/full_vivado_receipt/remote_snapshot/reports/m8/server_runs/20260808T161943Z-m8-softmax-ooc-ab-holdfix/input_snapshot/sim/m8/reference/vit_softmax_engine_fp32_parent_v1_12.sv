...
//   pass 0: find max
//   pass 1: recompute exp(x-max) and accumulate the sum
//   pass 2: recompute exp(x-max), normalize, and write
//
// Recomputing exp avoids a row-sized exponential buffer.  Latency increases,
// while the external request/result handshake and bit-exact FP32 result stay
// unchanged.
(* use_dsp = "no" *)
    ...
    always_comb begin
        ...
        case (state)
            ...
            STATE_OUTPUT_NORMALIZE: begin
                mul_operand_a = exponential_value;
