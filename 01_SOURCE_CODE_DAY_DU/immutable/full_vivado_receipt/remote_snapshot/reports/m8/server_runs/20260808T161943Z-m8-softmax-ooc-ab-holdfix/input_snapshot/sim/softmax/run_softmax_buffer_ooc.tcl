...
# The exact M7 parent exposed a single -0.020 ns internal hold path when the
# leaf used the generic Explore/Quick implementation recipe.  Do not waive
# that path: ask physical optimization to insert real data-path delay, then
# give the router the non-runtime-optimized exploration strategy.
phys_opt_design -directive ExploreWithAggressiveHoldFix
route_design -directive Explore

# A routed design can reveal clock-skew-dependent hold paths that were not
# visible accurately after placement.  If one remains, perform one bounded
# post-route aggressive hold repair and route the modified nets again.  The
# normal fail-closed timing gate below still rejects any residual violation.
...
if {[llength $initial_hold_paths] != 0} {
    ...
    if {$initial_whs ne "" && [expr {double($initial_whs) < 0.0}]} {
        puts "SOFTMAX_BUFFER_HOLD_REPAIR_BEGIN WHS=$initial_whs"
        ...
        if {[llength $repaired_hold_paths] != 0} {
            puts "SOFTMAX_BUFFER_HOLD_REPAIR_END WHS=[get_property SLACK [lindex $repaired_hold_paths 0]]"
        }
    }
}
