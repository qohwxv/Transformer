        ...
        begin
            ...
            if (a_nan || b_nan || ((a_inf || b_inf) && (a_zero || b_zero))) begin
                ...
                if (product[47]) begin
                    ...
                    exp_result = 32'(exp_a);
                    exp_result = exp_result + 32'(exp_b) - 127 + 1;
                end else begin
                    mantissa = product[46:23];
                    guard_bit = product[22];
                    sticky_bit = |product[21:0];
                    exp_result = 32'(exp_a);
                    exp_result = exp_result + 32'(exp_b) - 127;
                end

                round_up = guard_bit && (sticky_bit || mantissa[0]);
                rounded_mantissa =
                    {1'b0, mantissa} + {{24{1'b0}}, round_up};
    ...
    function automatic logic [31:0] fp32_add(
        ...
        begin
            ...
            if (a_nan || b_nan || (a_inf && b_inf && (sign_a != sign_b))) begin
                ...
                exp_difference = 32'(exp_large) - 32'(exp_small);
                exp_result = 32'(exp_large);
