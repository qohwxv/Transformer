...
patterns = [
    ...
    r"M7_MODE3_E01_M7_STATUS raw=[0-9a-fA-F]{8} running=0 snapshot=1 overflow=0 error=0 lc=[01] cs=[01] three=[01] both_claimed=1 claim_mask=3 bank_max=[12] fifo_max=[12] feeder_wait=[01] result_bp=[01] empty_wait=[01] full_wait=[01]",
    r"M7_MODE3_E01_M7_COUNTERS terms=117964800 disabled=2359296 enabled=115605504 dots=19200 results=19200 commits=921600 claims=921600 releases=921600 bank_max=[12] fifo_enqueue=19200 fifo_dequeue=19200 fifo_max=[12] load=[1-9][0-9]* compute=[1-9][0-9]* store=[1-9][0-9]* union=[1-9][0-9]* load_compute=[0-9]+ compute_store=[0-9]+ load_store=[0-9]+ three_way=[0-9]+",
    ...
    r"VIT_PHASE_E_AXI_E01_MODE3_REAL_RTL_STRUCTURAL_PASS checks=[1-9][0-9]* cycles=[1-9][0-9]* commands=4 external_u32=15351550 writes=453120 model_reads=14899198 input_reads=150528 scratch_reads=301824 numerical_status=PENDING_EXTERNAL_M6_CURRENT_ADDER_AND_BEHAVIORAL_ORACLES",
]
positions = []
