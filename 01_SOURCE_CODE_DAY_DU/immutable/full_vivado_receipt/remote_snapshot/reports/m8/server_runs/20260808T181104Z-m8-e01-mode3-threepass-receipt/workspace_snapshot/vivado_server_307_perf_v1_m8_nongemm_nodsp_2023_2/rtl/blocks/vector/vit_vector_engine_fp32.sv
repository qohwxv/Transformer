...
// multiplier-to-adder combinational path.  MODE_ADD bypasses the unused
// multiply state, while SCALE_MASK with masking disabled retires directly
// from the multiply state.  These M8 fast paths remove only arithmetic whose
// value is provably unobserved; the active FP32 operation and its rounding
// point remain unchanged.  The implementation still contains one shared
// fabric multiplier and one shared fabric adder.
