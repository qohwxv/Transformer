...
module vit_layernorm_engine_fp32 #(
    ...
    // One synchronous address/operation per memory and no array reset.  A
    // command overwrites every entry it will later consume before enabling a
    // local read, so stale contents cannot become architecturally visible.
    always_ff @(posedge clk) begin
        if (sample_buffer_write_enable)
            sample_buffer[row_affine_buffer_address] <= input_data;
        else if (sample_buffer_read_enable)
            sample_buffer_read_data <=
        ...
        if (gamma_beta_buffer_write_enable) begin
            ...
            gamma_buffer_read_data <=
                gamma_buffer[row_affine_buffer_address];
            beta_buffer_read_data <=
