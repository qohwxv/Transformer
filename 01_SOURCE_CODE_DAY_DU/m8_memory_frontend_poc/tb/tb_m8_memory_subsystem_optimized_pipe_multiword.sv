...
module tb_m8_memory_subsystem_optimized_pipe_multiword;
    ...
    // ============================================================
    // M2.7 DIRECT SCALAR AXI PROFILING
    // ============================================================

    integer m27_scalar_ar_count;
    integer m27_scalar_r_count;
    integer m27_scalar_ar_before_first_r;
    integer m27_max_scalar_outstanding;

    logic m27_first_scalar_r_seen;

    always_ff @(posedge aclk) begin
        if (!aresetn) begin
            m27_scalar_ar_count          <= 0;
            m27_scalar_r_count           <= 0;
            m27_scalar_ar_before_first_r <= 0;
            m27_max_scalar_outstanding   <= 0;
            m27_first_scalar_r_seen      <= 1'b0;

        end else begin

            if (dut.u_mem_adapter.scalar_ar_fire) begin
                m27_scalar_ar_count <=
                    m27_scalar_ar_count + 1;

                if (!m27_first_scalar_r_seen)
                    m27_scalar_ar_before_first_r <=
                        m27_scalar_ar_before_first_r + 1;
            end

            if (dut.u_mem_adapter.scalar_r_fire) begin
                m27_scalar_r_count <=
                    m27_scalar_r_count + 1;

                m27_first_scalar_r_seen <= 1'b1;
            end

            if (dut.u_mem_adapter.scalar_meta_count >
                m27_max_scalar_outstanding)
                m27_max_scalar_outstanding <=
                    dut.u_mem_adapter.scalar_meta_count;
        end
    end
    ...
    initial begin
        ...
        // No result-store traffic in Test 5A
        ...
        // Packed B from MODEL/PARAM
        active_cmd.route.src1_space =
            PHASE_E_MEM_PARAM;

        // Bias from MODEL/PARAM
        ...
        $display("");
        $display("================================================");
        $display("TEST 5A: GEMM PACKED COLD PANEL");
        $display("geometry       = R8 / C2 / K16");
        $display("A words        = 128");
        $display("packed B words = 16");
        $display("bias words     = 2");
        $display("total gather   = 146");
        $display(
            "flags          = 0x%02h",
            active_cmd.header.flags
        );
        ...
        // Emulate GEMM scheduler holding operand request
        ...
        fork
            ...
            // NORMAL COMPLETION
            // ====================================================
            begin : WAIT_PACKED_PANEL

                wait (gemm_data_valid === 1'b1);

                $display("");
                $display("gemm_data_valid asserted");
                ...
                if (
                    ...
                    $display(
                        "FAIL bias[0] exp=3f800000 got=%08h",
                        gemm_bias_data[31:0]
                    );
                ...
                if (
                    ...
                    $display(
                        "FAIL bias[1] exp=40000000 got=%08h",
                        gemm_bias_data[63:32]
                    );
                ...
                $display(
                    "PASS: bias[0..1] correct"
                );


                // Drop request before frontend starts another panel
                @(negedge aclk);
                selected_data_request = 1'b0;
                execute = 1'b0;
                ...
                $display(
                    "A cache lookup/hit/miss = %0d/%0d/%0d",
                    a_cache_lookup_count,
                    a_cache_hit_count,
                    a_cache_miss_count
                );
                ...
                // ================================================
                // Hard logical-contract check
                // ================================================
                if (logical_read_count != 146) begin

                    $display(
                        "FAIL: expected 146 logical reads"
                    );
                ...
                $display(
                    "scalar AR before first R = %0d",
                    m27_scalar_ar_before_first_r
                );
                $display(
                    "max scalar outstanding   = %0d",
                    m27_max_scalar_outstanding
                );
                $display(
                    "-------------------------------------------"
                ...
                if (m27_scalar_ar_count < 2) begin
                    $display(
                        "FAIL: scalar AXI pipe not exercised"
                    );
                    $fatal(1);
                end

                if (m27_scalar_ar_count !=
                    m27_scalar_r_count) begin
                    $display(
                        "FAIL: scalar AR/R count mismatch"
                    );
                    $fatal(1);
                end

                if (m27_scalar_ar_before_first_r < 2) begin
                    $display(
                        "FAIL: second scalar AR did not precede first R"
                    );
                    $fatal(1);
                end

                if (m27_max_scalar_outstanding != 2) begin
                    $display(
                        "FAIL: scalar outstanding never reached 2"
                    );
                    $fatal(1);
                end
                ...
                $display("");
                $display(
                    "M2.7 TRUE SCALAR OUTSTANDING TEST PASS"
                );
