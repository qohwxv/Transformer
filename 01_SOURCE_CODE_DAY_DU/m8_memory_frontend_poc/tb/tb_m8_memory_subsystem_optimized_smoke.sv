...
module tb_m8_memory_subsystem_optimized_smoke;
    ...
    // ------------------------------------------------------------
    initial begin
        // Known defaults for every frontend input.
        aresetn = 1'b0;
        ...
        // Layout production dispatch uses one read / one write word.
        ...
        // Layout test address
        layout_source_address = TEST_WORD_ADDR;
        ...
        // --------------------------------------------------------
        // Build minimal Layout command.
        // Only opcode and src0 memory space matter for this read.
        // --------------------------------------------------------
        ...
        // Wait several clocks in reset.
        repeat (8) @(posedge aclk);

        // Seed simulated physical DDR:
        //
        // physical =
        // SCRATCH_BASE + (logical_word_addr << 2)
        //
        u_ddr.poke_u32(
            SCRATCH_BASE + (TEST_WORD_ADDR << 2),
            TEST_VALUE
        );

        $display("");
        $display("==============================================");
        $display("M8 MEMORY SUBSYSTEM OPTIMIZED SMOKE TEST");
        $display("TEST: LAYOUT SINGLE-WORD READ");
        $display("logical addr = %0d", TEST_WORD_ADDR);
        $display("physical addr = 0x%0h",
                 SCRATCH_BASE + (TEST_WORD_ADDR << 2));
        $display("expected = 0x%08h", TEST_VALUE);
        $display("==============================================");
        ...
        // Command accept pulse resets frontend command-local state.
        @(negedge aclk);
        command_accept = 1'b1;
        ...
        // Emulate controller EXECUTE state.
        execute = 1'b1;

        // Emulate Layout engine holding data_request until data_valid.
        selected_data_request = 1'b1;

        // Timeout protection.
        fork
            begin : WAIT_FOR_DATA
                wait (layout_data_valid === 1'b1);

                if (layout_source_data !== TEST_VALUE) begin
                    $display("FAIL: DATA MISMATCH");
                    $display("expected = 0x%08h", TEST_VALUE);
                    $display("actual   = 0x%08h", layout_source_data);
                    $fatal(1);
                end

                $display("PASS: layout_source_data = 0x%08h",
                         layout_source_data);

                // Drop request before frontend can re-enter a new read.
                @(negedge aclk);
                selected_data_request = 1'b0;
                execute = 1'b0;
                ...
                if (memory_error_latched) begin
                    $display("FAIL: memory_error_latched = 1");
                    $fatal(1);
                end

                if (axi_r_protocol_error_o) begin
                    $display("FAIL: AXI R protocol error");
                    $fatal(1);
                end

                $display("");
                $display("--------- OPTIMIZED SMOKE COUNTERS ---------");
                $display("cycles              = %0d", cycle_count);
                $display("logical read words  = %0d", logical_read_count);
                $display("AR handshakes       = %0d", ar_count);
                $display("R beat handshakes   = %0d", r_beat_count);
                $display("narrow R beats      = %0d", narrow_read_count);
                $display("linefill_start       = %0b", linefill_start_o);
                $display("read_outstanding     = %0d", read_outstanding_o);
                $display("-------------------------------------------");

                if (logical_read_count != 1) begin
                    $display("FAIL: expected one logical read");
                    $fatal(1);
                end

                if (ar_count != 1) begin
                    $display("FAIL: expected one AXI AR transaction");
                    $fatal(1);
                end

                if (r_beat_count != 1) begin
                    $display("FAIL: expected one AXI R beat");
                    $fatal(1);
                end

                $display("");
                $display("==============================================");
                $display("OPTIMIZED SMOKE TEST PASS");
                $display("==============================================");
            ...
            begin : TIMEOUT
                ...
                $display("debug_mem_state      = %0d", debug_mem_state);
                $display("memory_error_latched = %0b",
                         memory_error_latched);
                $display("AR count             = %0d", ar_count);
                $display("R beat count         = %0d", r_beat_count);
