...
module tb_m8_memory_subsystem_gemm_outstanding2;
    ...
    // ============================================================
    // GEMM NON-PACKED B / 32-WORD / OUTSTANDING=2 TEST
    // ============================================================

    localparam logic [31:0] WEIGHT_BASE = 32'd64;

    // R8/C2/K16
    localparam integer GEMM_A_WORDS_TB = 8 * 16;  // 128
    localparam integer GEMM_B_WORDS_TB = 2 * 16;  // 32
    localparam integer GEMM_BIAS_WORDS_TB = 2;
    localparam integer GEMM_READ_WORDS_TB =
        GEMM_A_WORDS_TB +
        GEMM_B_WORDS_TB +
        GEMM_BIAS_WORDS_TB;                       // 162
    ...
    initial begin
        ...
        // Exact frontend GEMM gather geometry:
        // 128 A + 32 B + 2 bias = 162 logical positions.
        read_word_count  = GEMM_READ_WORDS_TB;
        write_word_count = 16'd16;

        // --------------------------------------------------------
        // GEMM live coordinates
        // --------------------------------------------------------
        ...
        // No result traffic in this test.
        ...
        active_cmd.src0_base = 32'd0;
        active_cmd.src1_base = WEIGHT_BASE;
        active_cmd.src2_base = 32'd0;
        ...
        // M=0 intentionally:
        // all 128 A candidates become not-needed.
        active_cmd.dim1 = 32'd0;
        ...
        // Keep all coordinate increments at zero for this isolated
        // B-panel test. Current live batch/output/k bases are zero.
        active_cmd.stride0 = 32'd0;
        active_cmd.stride1 = 32'd0;
        ...
        // ========================================================
        // SEED 32 CONTIGUOUS NON-PACKED B WORDS
        //
        // MODEL_BASE = 0
        // WEIGHT_BASE = 64 words
        //
        // physical first byte:
        // 64 * 4 = 0x100
        //
        ...
        $display("");
        $display("==============================================");
        $display("TEST: GEMM NON-PACKED B / OUTSTANDING=2");
        $display("B words        = 32");
        $display("B logical base = %0d", WEIGHT_BASE);
        $display(
            "B physical base = 0x%0h",
            MODEL_BASE + (WEIGHT_BASE << 2)
        );
        $display("Expected AXI:");
        $display("  2 AR bursts");
        $display("  4 beats/burst");
        $display("  8 total R beats");
        $display("  max outstanding = 2");
        $display("==============================================");
        ...
        fork
            begin : WAIT_GEMM_DATA

                wait (gemm_data_valid === 1'b1);

                // =================================================
                // VERIFY ALL 32 B WORDS
                // =================================================
                ...
                for (i = 0; i < 32; i = i + 1) begin
                    if (
                        ...
                        $display(
                            "FAIL B[%0d] expected=%08h actual=%08h",
                            i,
                            32'hB600_0000 + i,
                            gemm_weight_data[i*32 +: 32]
                        );
                ...
                $display(
                    "PASS: all 32 GEMM B words match"
                );

                // Stop native operand request before frontend can
                // begin another bundle.
                @(negedge aclk);
                selected_data_request = 1'b0;
                execute = 1'b0;
                ...
                if (logical_read_count != 32) begin
                    $display(
                        "FAIL: expected 32 logical B reads"
                    );
                    $fatal(1);
                end
                ...
                if (ar_count != 2) begin
                    $display(
                        "FAIL: expected 2 AXI AR bursts"
                    );
                    $fatal(1);
                end

                if (r_beat_count != 8) begin
                    $display(
                        "FAIL: expected 8 AXI R beats"
                    );
                    $fatal(1);
                end
                ...
                if (linefill_start_count != 1) begin
                    $display(
                        "FAIL: expected 1 linefill"
                    );
                    $fatal(1);
                end

                if (full_r_beat_count != 8) begin
                    $display(
                        "FAIL: expected 8 full R beats"
                    );
                    $fatal(1);
                end
                ...
                if (max_read_outstanding_seen != 2) begin
                    $display(
                        "FAIL: read outstanding never reached 2"
                    );
                    $fatal(1);
                end
                ...
                $display("");
                $display("==============================================");
                $display("GEMM OUTSTANDING=2 TEST PASS");
                $display("==============================================");
            ...
            begin : TIMEOUT
                ...
                $display("");
                $display(
                    "FAIL: GEMM OUTSTANDING TEST TIMEOUT"
                );
                $display(
                    "debug_mem_state = %0d",
                    debug_mem_state
