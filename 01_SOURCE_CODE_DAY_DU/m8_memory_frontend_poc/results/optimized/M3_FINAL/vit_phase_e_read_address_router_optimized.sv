...
(* use_dsp = "no" *)
module vit_phase_e_read_address_router_optimized #(
    ...
    // A speculative line fill is legal only when every advertised word is
    // guaranteed to be consumed before an engine can publish a write.  M5
    // established that contract for blocked immutable model-B storage.  M8
    // also admits one bounded Vector/GELU gather (at most VECTOR_LANES words):
    // both engines latch the complete gather before exposing result_valid.
    // The count is the number of physically contiguous FP32 words beginning
    // at candidate_address.
    ...
    integer contiguous_lane;
    logic contiguous_lane_run;
    logic [5:0] contiguous_lane_count;
    logic [32:0] candidate_element_index_wide;
    logic [32:0] candidate_remaining_words_wide;
    logic [5:0] candidate_lanes_remaining;
    ...
    always_comb begin
        ...
        candidate_col = 0;
        candidate_lane = 0;
        contiguous_lane_run = 1'b0;
        contiguous_lane_count = 6'd0;
        candidate_element_index_wide = 33'd0;
        candidate_remaining_words_wide = 33'd0;
        candidate_lanes_remaining = 6'd0;
        ...
        case (active_cmd.header.opcode)
            PHASE_E_OP_GEMM: begin
                if (word_index_u32 < GEMM_A_WORDS_U32) begin
                    ...
                    // M3.2A:
                    // GEMM-A is physically contiguous only within the
                    // current PE_LANES row.  Do not advertise words across
                    // the row boundary because stride1 may exceed PE_LANES.
                    //
                    // GEMM_CACHE_SAFE is the software contract that src0
                    // remains immutable and cannot be clobbered by the
                    // command destination while this bounded gather occurs.
                    //
                    // Clamp again at the semantic K tail so speculative
                    // line-fill never crosses dim2.
                    if (candidate_needed &&
                        ((active_cmd.header.flags &
                          PHASE_E_FLAG_GEMM_CACHE_SAFE) != 0)) begin

                        candidate_lanes_remaining =
                            6'(PE_LANES - candidate_lane);

                        candidate_remaining_words_wide =
                            {1'b0, active_cmd.dim2} -
                            (
                                {1'b0, gemm_k_base} +
                                {1'b0, candidate_lane[31:0]}
                            );

                        candidate_read_ahead_safe = 1'b1;

                        if (candidate_remaining_words_wide <
                            {27'd0, candidate_lanes_remaining})
                            candidate_contiguous_words =
                                candidate_remaining_words_wide[5:0];
                        else
                            candidate_contiguous_words =
                                candidate_lanes_remaining;
                    end
            ...
            PHASE_E_OP_VECTOR: begin
                if (word_index_u32 < VECTOR_LANES_U32) begin
                    ...
                    candidate_element_index_wide =
                        {1'b0, vector_element_base} +
                        {1'b0, candidate_lane[31:0]};
                    candidate_needed =
                ...
                // A/B aliases and in-place destinations are safe here: the
                // frontend gathers every requested lane before the Vector
                // engine can expose a result.  Clamp the hint to the current
                // packed gather and the active command tail.
                if (candidate_needed) begin
                    candidate_lanes_remaining =
                        6'(VECTOR_LANES - candidate_lane);
                    candidate_remaining_words_wide =
                        {1'b0, active_cmd.dim0} -
                        candidate_element_index_wide;
                    candidate_read_ahead_safe = 1'b1;
                    if (candidate_remaining_words_wide <
                        {27'd0, candidate_lanes_remaining})
            ...
            PHASE_E_OP_GELU: begin
                ...
                // GELU masks are normally a low contiguous prefix.  Count the
                // actual consecutive asserted lanes so a malformed/sparse
                // mask never causes an otherwise skipped word to be fetched.
                contiguous_lane_run = 1'b1;
                for (contiguous_lane = 0;
                     contiguous_lane < VECTOR_LANES;
                     contiguous_lane = contiguous_lane + 1) begin
                    if (contiguous_lane < candidate_lane) begin
                    end else if (contiguous_lane_run &&
                                 gelu_data_lane_mask[contiguous_lane]) begin
                        contiguous_lane_count =
                            contiguous_lane_count + 1'b1;
                    end else begin
                        contiguous_lane_run = 1'b0;
                    end
                end
                if (candidate_needed && (contiguous_lane_count != 0)) begin
                    candidate_read_ahead_safe = 1'b1;
                    candidate_contiguous_words = contiguous_lane_count;
                end
            end
