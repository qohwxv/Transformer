...
// Scalar writes retain the original narrow one-beat path.  M7.2A also
// accepts an explicitly qualified four-word request and emits one native
// 128-bit AXI write beat.  No write is acknowledged before its BRESP commits.
//
...
module vit_phase_e_axi_mem_adapter_optimized #(
    ...
    // M7.2A optional native-width write.
    input  logic                          req_write_wide,
    input  logic [127:0]                  req_write_data_wide,
    input  logic [15:0]                   req_write_strobe_wide,
    ...
    typedef enum logic [3:0] {
        ...
        STATE_POISON_FLUSH,
        STATE_POISON,
        // M2.7: decoupled scalar AR issue / R retire.
        // Appended to preserve all existing state encodings.
        STATE_SCALAR_READ_PIPE,

        // M7.4: a four-word logical wide request whose physical
        // address is not naturally AXI128 aligned is committed as
        // four exact scalar writes, but returns only one logical
        // response to the frontend.
        STATE_WIDE_FALLBACK_ISSUE,
        STATE_WIDE_FALLBACK_RESPONSE
    } state_t;
    ...
    logic         req_fifo_write_wide [0:1];
    logic [127:0] req_fifo_write_data_wide [0:1];
    logic [15:0]  req_fifo_write_strobe_wide [0:1];
    ...
    logic         core_req_write_wide;
    logic [127:0] core_req_write_data_wide;
    logic [15:0]  core_req_write_strobe_wide;
    ...
    // M7.2A wide-write safety checks.
    logic [32:0] request_wide_end_word;
    logic [64:0] request_wide_end_address_wide;
    logic        request_wide_address_fits_axi;
    logic        request_wide_write_valid;
    logic        write_wide_hold;
    ...
    // M2.7 scalar AXI metadata queue.
    // Same AXI ID preserves R ordering, therefore only the 32-bit
    // lane associated with each accepted AR must be retained.
    logic [1:0] scalar_meta_lane [0:1];
    logic       scalar_meta_read_pointer;
    logic       scalar_meta_write_pointer;
    logic [1:0] scalar_meta_count;
    logic [1:0] scalar_meta_count_next;

    // First request may enter through the adapter fall-through path.
    // Subsequent scalar requests remain in req_fifo until their AR
    // handshake actually occurs.
    logic       scalar_first_pending;
    logic       scalar_poison_q;

    logic       scalar_fifo_issue_valid;
    logic       scalar_pipe_arvalid;
    logic       scalar_ar_from_fifo;
    logic       scalar_ar_fire;
    logic       scalar_r_fire;

    logic [AXI_ADDR_WIDTH-1:0] scalar_issue_address;
    logic [1:0] scalar_issue_lane;
    logic [2:0] scalar_reserved_slots;

    logic       scalar_rsp_framing_error;
    logic       scalar_rsp_error;
    logic [31:0] scalar_rsp_data;
    ...
    // M7.4 misaligned native-width fallback.
    logic        request_wide_fallback_valid;
    logic [127:0] wide_fallback_data_hold;
    logic [15:0]  wide_fallback_strobe_hold;
    logic [1:0]   wide_fallback_word_index;
    logic [1:0]   wide_fallback_lane;

    logic [AXI_ADDR_WIDTH-1:0]
        wide_fallback_next_address;

    logic [1:0]
        wide_fallback_next_lane;
    ...
    assign core_req_write_wide =
        req_bypass_start ? req_write_wide :
        req_fifo_write_wide[req_fifo_read_pointer];

    assign core_req_write_data_wide =
        req_bypass_start ? req_write_data_wide :
        req_fifo_write_data_wide[req_fifo_read_pointer];

    assign core_req_write_strobe_wide =
        req_bypass_start ? req_write_strobe_wide :
        req_fifo_write_strobe_wide[req_fifo_read_pointer];
    ...
    always_comb begin
        ...
        // Four-word native write must remain inside the configured
        // memory aperture and fit in one naturally aligned AXI128 beat.
        request_wide_end_word =
            {1'b0, core_req_word_address} + 33'd3;

        request_wide_end_address_wide =
            physical_address_wide + 65'd15;

        request_wide_address_fits_axi =
            !request_wide_end_address_wide[64];

        for (address_bit = AXI_ADDR_WIDTH;
             address_bit < 64;
             address_bit = address_bit + 1)
            if (request_wide_end_address_wide[address_bit])
                request_wide_address_fits_axi = 1'b0;

        request_wide_write_valid =
            request_is_valid &&
            core_req_write &&
            core_req_write_wide &&
            (core_req_space == MEM_SCRATCH) &&
        ...
        // M7.4:
        // A physically misaligned four-word request is still legal
        // when every constituent u32 is in range.  The adapter will
        // commit it as four scalar AXI transactions.
        //
        // Do NOT use this path for aperture overflow or AXI address
        // overflow; those remain genuine request errors.
        request_wide_fallback_valid =
            request_is_valid &&
            core_req_write &&
            core_req_write_wide &&
            (core_req_space == MEM_SCRATCH) &&
    ...
    // ============================================================
    // M2.7 SCALAR AXI ISSUE / RETIRE PIPE
    // ============================================================

    // Every accepted scalar AR reserves one eventual logical response.
    // Do not allow the sum of buffered responses and AXI outstanding
    // reads to exceed the physical response FIFO depth.
    assign scalar_reserved_slots =
        {1'b0, rsp_fifo_count} +
        {1'b0, scalar_meta_count};

    // Once the first fall-through request has issued, a queued request
    // may issue directly from req_fifo only when it is an exact narrow
    // scalar read.  Writes, line-fill candidates, cache hits and invalid
    // requests remain ordering barriers and are handled by STATE_IDLE
    // after older scalar AXI traffic drains.
    assign scalar_fifo_issue_valid =
        (state == STATE_SCALAR_READ_PIPE) &&
        !scalar_first_pending &&
        !scalar_poison_q &&
        (req_fifo_count != 0) &&
        request_is_valid &&
        !core_req_write &&
        !request_hits_line &&
        !request_can_linefill &&
        (scalar_reserved_slots < 3'd2);

    assign scalar_issue_address =
        scalar_first_pending ?
        scalar_address_hold :
        physical_address_wide[AXI_ADDR_WIDTH-1:0];

    assign scalar_issue_lane =
        scalar_first_pending ?
        scalar_lane_hold :
        physical_address_wide[3:2];

    assign scalar_pipe_arvalid =
        (state == STATE_SCALAR_READ_PIPE) &&
        !scalar_poison_q &&
        (scalar_reserved_slots < 3'd2) &&
        (scalar_first_pending || scalar_fifo_issue_valid);

    assign scalar_ar_from_fifo =
        scalar_pipe_arvalid && !scalar_first_pending;

    assign scalar_ar_fire =
        scalar_pipe_arvalid && m_axi_arready;

    // R is accepted only when metadata exists and an ordered logical
    // response can be stored without overflowing rsp_fifo.
    assign scalar_r_fire =
        (state == STATE_SCALAR_READ_PIPE) &&
        (scalar_meta_count != 0) &&
        m_axi_rvalid &&
        m_axi_rready;
    ...
    always_comb begin
        scalar_meta_count_next = scalar_meta_count;

        case ({scalar_ar_fire, scalar_r_fire})
            2'b10:
                scalar_meta_count_next =
                    scalar_meta_count + 2'd1;
    ...
    // Every scalar request is ARLEN=0, therefore RLAST must be asserted
    // on the single returned beat.
    assign scalar_rsp_framing_error =
        (m_axi_rid != {AXI_ID_WIDTH{1'b0}}) ||
        !m_axi_rlast;

    // Once framing certainty is lost, all already-issued reads are
    // drained as ordered error tokens before queued requests are flushed.
    assign scalar_rsp_error =
        scalar_poison_q ||
        scalar_rsp_framing_error ||
        (m_axi_rresp != AXI_RESP_OKAY);

    assign scalar_rsp_data =
        (scalar_poison_q || scalar_rsp_framing_error) ?
        32'd0 :
        m_axi_rdata[
            scalar_meta_lane[scalar_meta_read_pointer] * 32
            +: 32
        ];
    ...
    assign req_fifo_pop =
        req_queued_start ||
        req_poison_flush ||
        (scalar_ar_fire && scalar_ar_from_fifo);
    ...
    // ============================================================
    // M7.4B FALLBACK ADDRESS/LANE ADVANCE
    //
    // Example:
    //   ...004 lane1
    //   ...008 lane2
    ...
    //   ...010 lane0
    //
    // Deriving the lane from the actual physical address prevents
    // a lane3+1 expression from becoming shift amount 4/128 bits.
    // ============================================================

    assign wide_fallback_next_address =
        scalar_address_hold +
        {{(AXI_ADDR_WIDTH-3){1'b0}}, 3'd4};

    assign wide_fallback_next_lane =
        wide_fallback_next_address[3:2];
    ...
    assign m_axi_awvalid =
        ((state == STATE_WRITE_ISSUE) ||
         (state == STATE_WIDE_FALLBACK_ISSUE)) &&
        !aw_complete;
    ...
    assign m_axi_bready =
        ...
        (state == STATE_WIDE_FALLBACK_RESPONSE);
    ...
    always_comb begin
        ...
        if (state == STATE_SCALAR_READ_PIPE) begin
            m_axi_araddr = scalar_issue_address;
            m_axi_arlen = 8'd0;
            m_axi_arsize = 3'b010;

        end else if (state == STATE_LINE_FILL) begin
            m_axi_araddr =
    ...
    always_comb begin
        ...
        else if (state == STATE_SCALAR_READ_PIPE)
            read_outstanding_o = scalar_meta_count;
        else
            read_outstanding_o = 2'd0;
    end
    ...
    always_ff @(posedge aclk) begin : adapter_sequential
        ...
        if (!aresetn) begin
            ...
            req_fifo_write_wide[0] <= 1'b0;
            req_fifo_write_wide[1] <= 1'b0;
            req_fifo_write_data_wide[0] <= 128'd0;
            req_fifo_write_data_wide[1] <= 128'd0;
            req_fifo_write_strobe_wide[0] <= 16'd0;
            req_fifo_write_strobe_wide[1] <= 16'd0;
            ...
            scalar_meta_lane[0] <= 2'd0;
            scalar_meta_lane[1] <= 2'd0;
            scalar_meta_read_pointer <= 1'b0;
            scalar_meta_write_pointer <= 1'b0;
            scalar_meta_count <= 2'd0;
            scalar_first_pending <= 1'b0;
            scalar_poison_q <= 1'b0;

            write_data_hold <= 128'd0;
            write_strobe_hold <= 16'd0;
            write_wide_hold <= 1'b0;

            wide_fallback_data_hold <= 128'd0;
            wide_fallback_strobe_hold <= 16'd0;
            wide_fallback_word_index <= 2'd0;
            wide_fallback_lane <= 2'd0;
            ...
            if (req_fifo_push) begin
                ...
                req_fifo_write_wide[req_fifo_write_pointer] <=
                    req_write_wide;
                req_fifo_write_data_wide[req_fifo_write_pointer] <=
                    req_write_data_wide;
                req_fifo_write_strobe_wide[req_fifo_write_pointer] <=
                    req_write_strobe_wide;
            ...
            case (state)
                    ...
                    if (req_core_start) begin
                        ...
                        if (!request_is_valid) begin
                            ...
                            if (
                                ...
                                // ------------------------------------
                                // M7.4 physical-misalignment fallback
                                //
                                // Frontend already consumed four
                                // logical result words.  Preserve that
                                // one-request/one-response contract,
                                // but commit the payload as four exact
                                // scalar AXI writes.
                                // ------------------------------------
                                wide_fallback_data_hold <=
                                    core_req_write_data_wide;

                                wide_fallback_strobe_hold <=
                                    core_req_write_strobe_wide;

                                wide_fallback_word_index <= 2'd0;
                                wide_fallback_lane <= request_lane;

                                // First u32.
                                write_data_hold <=
                                    {96'd0,
                                     core_req_write_data_wide[31:0]}
                                    << (request_lane * 32);

                                write_strobe_hold <=
                                    {12'd0,
                                     core_req_write_strobe_wide[3:0]}
                                    << (request_lane * 4);
                                ...
                                // Invalid for reasons other than
                                // native-width alignment, e.g. memory
                                // aperture or AXI-address overflow.
                                core_rsp_error <= 1'b1;
                                write_wide_hold <= 1'b0;
                                state <= STATE_LOCAL_RESPONSE;

                            end else begin
                                if (core_req_write_wide) begin
                                    write_data_hold <=
                                        core_req_write_data_wide;
                                    write_strobe_hold <=
                                        core_req_write_strobe_wide;
                                    write_wide_hold <= 1'b1;
                                end else begin
                                    // Historical scalar write.
                                    write_data_hold <=
                            ...
                            // First scalar request was already consumed by
                            // req_core_start/fall-through.  Hold its physical
                            // address/lane until AR accepts it.
                            scalar_first_pending <= 1'b1;
                            scalar_meta_read_pointer <= 1'b0;
                            scalar_meta_write_pointer <= 1'b0;
                            scalar_meta_count <= 2'd0;
                            scalar_poison_q <= 1'b0;
                ...
                // ====================================================
                // M2.7 TRUE SCALAR OUTSTANDING PIPE
                //
                // AR issue and R retirement are independent.
                // Same-ID AXI ordering maps each R beat to the oldest
                // scalar metadata entry.
                // ====================================================
                STATE_SCALAR_READ_PIPE: begin

                    // -------------------------------
                    // AR ISSUE
                    // -------------------------------
                    if (scalar_ar_fire) begin
                        scalar_meta_lane[
                            scalar_meta_write_pointer
                        ] <= scalar_issue_lane;

                        scalar_meta_write_pointer <=
                            ~scalar_meta_write_pointer;

                        if (scalar_first_pending)
                            scalar_first_pending <= 1'b0;
                    end
                    ...
                    // R RETIRE
                    // -------------------------------
                    if (scalar_r_fire) begin
                        narrow_r_beat_o <= 1'b1;

                        scalar_meta_read_pointer <=
                            ~scalar_meta_read_pointer;

                        if (scalar_rsp_framing_error) begin
                            axi_r_protocol_error_o <= 1'b1;
                            scalar_poison_q <= 1'b1;

                        end else if (
                            m_axi_rresp != AXI_RESP_OKAY
                        ) begin
                            // BRESP/RRESP error remains a bounded logical
                            // response error; only malformed framing poisons
                            // transaction-boundary certainty.
                            axi_r_protocol_error_o <= 1'b1;
                        end
                    end

                    // Accepted ARs and retired Rs may occur together.
                    scalar_meta_count <=
                        scalar_meta_count_next;

                    scalar_read_outstanding <=
                        (scalar_meta_count_next != 0);

                    // A framing error invalidates attribution certainty.
                    // Drain every already-issued scalar read as an ordered
                    // error token before flushing still-queued requests.
                    if (
                        (scalar_poison_q ||
                         (scalar_r_fire &&
                          scalar_rsp_framing_error)) &&
                    ...
                    // If the queued head is another eligible scalar read,
                    // remain here and issue it as soon as ARREADY/capacity
                    // permit.  Otherwise return to IDLE after all older AXI
                    // reads retire.
                    end else if (
                        !scalar_poison_q &&
                        !(scalar_r_fire &&
                ...
                // ====================================================
                // M7.4 MISALIGNED FOUR-WORD FALLBACK
                // ====================================================

                STATE_WIDE_FALLBACK_ISSUE: begin
                ...
                STATE_WIDE_FALLBACK_RESPONSE: begin
                    if (m_axi_bvalid && m_axi_bready) begin
                        ...
                        if (
                            ...
                            wide_fallback_word_index == 2'd3
                        ) begin

                            // All four scalar commits completed.
                            // Return ONE logical response upstream.
                            core_rsp_data <= 32'd0;
                            core_rsp_error <= 1'b0;
                            ...
                            wide_fallback_word_index <=
                                wide_fallback_word_index + 2'd1;

                            // Select next payload word.
                            case (wide_fallback_word_index)
